...
static int hub_int_cb(struct usb_device *const udev, struct uhc_transfer *const xfer)
{
	...
	k_mutex_lock(&hub->lock, K_FOREVER);
	...
	atomic_or(&hub->changes, changes);
	k_work_submit_to_queue(&hub_wq, &hub->work);
...
{
	if (status & USB_HUB_PORT_STAT_LOW_SPEED) {
		return USB_SPEED_SPEED_LS;
	}

	if (status & USB_HUB_PORT_STAT_HIGH_SPEED) {
			  ...
			  uint16_t *const status)
{
	...
	for (uint32_t t = 0U; t < HUB_RESET_TIMEOUT_MS; t += HUB_RESET_POLL_MS) {
		...
		if (change & USB_HUB_PORT_CHANGE_RESET) {
...
static void hub_port_connect(struct usbh_hub_data *const hub, const uint8_t port)
{
	...
	if (!(status & USB_HUB_PORT_STAT_CONNECTION) ||
	    (change & USB_HUB_PORT_CHANGE_CONNECTION)) {
...
static void hub_port_event(struct usbh_hub_data *const hub, const uint8_t port)
{
	...
	if (change & USB_HUB_PORT_CHANGE_CONNECTION) {
	...
	if (change & USB_HUB_PORT_CHANGE_ENABLE) {
	...
	if (change & USB_HUB_PORT_CHANGE_SUSPEND) {
		(void)usbh_req_clear_hcfs(hub->udev, port, USB_HCFS_C_PORT_SUSPEND);
	}

	if (change & USB_HUB_PORT_CHANGE_OVER_CURRENT) {
...
static void hub_status_event(struct usbh_hub_data *const hub)
{
	...
	if (change & USB_HUB_CHANGE_LOCAL_POWER) {
		(void)usbh_req_clear_hcfs(hub->udev, 0, USB_HCFS_C_HUB_LOCAL_POWER);
	}

	if (change & USB_HUB_CHANGE_OVER_CURRENT) {
...
static int hub_read_descriptor(struct usbh_hub_data *const hub)
{
	...
	hub->num_ports = MIN(desc.bNbrPorts, CONFIG_USBH_HUB_MAX_PORTS);
			  ...
			  struct usb_device *const udev, const uint8_t iface)
{
	...
	atomic_or(&hub->changes, GENMASK(hub->num_ports, 1));
	k_work_submit_to_queue(&hub_wq, &hub->work);
...
static int usbh_hub_removed(struct usbh_class_data *const c_data)
{
	...
	k_work_cancel_sync(&hub->work, &sync);
...
static struct usbh_class_filter usbh_hub_filters[] = {
	{.flags = USBH_CLASS_MATCH_CODE_TRIPLE, .class = USB_BCC_HUB, .sub = 0, .proto = 0},
	{.flags = USBH_CLASS_MATCH_CODE_TRIPLE, .class = USB_BCC_HUB, .sub = 0, .proto = 1},
	{.flags = USBH_CLASS_MATCH_CODE_TRIPLE, .class = USB_BCC_HUB, .sub = 0, .proto = 2},
	{0},
};
