...
static inline uint32_t ohci_phys_addr(const void *addr)
{
	return (uint32_t)k_mem_phys_addr((void *)addr);
}

/* Write back a buffer the host controller is going to read */
static inline void ohci_buf_flush(const void *addr, size_t len)
{
 ...
 */
static size_t ohci_chunk_len(uintptr_t addr, size_t remaining)
{
	size_t max_len = (0x1000U - (addr & 0xfffU)) + 0x1000U;
				 ...
				 bool dir_in, bool interrupt_on_done)
{
	...
	td->hw.flags = flags;
	td->hw.cbp = len ? ohci_phys_addr((void *)data) : 0U;
	td->hw.be = len ? ohci_phys_addr((void *)(data + len - 1U)) : 0U;
	td->data = len ? ohci_phys_addr((void *)data) : 0U;
	td->len = (uint16_t)len;
...
static int ohci_build_control_chain(struct ohci_xfer_slot *slot, struct uhc_transfer *xfer)
{
	...
	ohci_fill_td(&slot->tds[td_idx++], (uintptr_t)slot->setup, sizeof(slot->setup),
...
static int ohci_build_bulk_chain(struct ohci_xfer_slot *slot, struct uhc_transfer *xfer)
{
	...
	if (remaining == 0U) {
		...
		ohci_fill_td(&slot->tds[0], 0U, 0U, OHCI_TD_DP_OUT, OHCI_TD_T_TOGGLE_ED,
			     false, false, true);
		slot->td_count = 2U;
		return 0;
	}
	 ...
	 */
	for (uint16_t i = 0U; dir_in && i < (td_idx - 1U); i++) {
