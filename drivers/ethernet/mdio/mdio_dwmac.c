...
struct dwmac_mdio_clk_range {
	uint32_t rate;
	uint8_t csr_clk_indx;
};

static const struct dwmac_mdio_clk_range dwmac_mdio_clk_ranges[] = {
	{ 800000000U, UINT8_MAX },
	{ 500000000U, 7U },
	{ 300000000U, 6U },
	{ 250000000U, 5U },
	{ 150000000U, 4U },
...
static int dwmac_mdio_csr_clk_idx_from_rate(uint32_t clock_rate, uint8_t *csr_clk_indx)
{
	for (size_t i = 0; i < ARRAY_SIZE(dwmac_mdio_clk_ranges); i++) {
		if (clock_rate > dwmac_mdio_clk_ranges[i].rate) {
			if (dwmac_mdio_clk_ranges[i].csr_clk_indx == UINT8_MAX) {
				return -EINVAL;
			}

			*csr_clk_indx = dwmac_mdio_clk_ranges[i].csr_clk_indx;
			return 0;
		}
	}
...
static int dwmac_mdio_resolve_csr_clk_idx(const struct dwmac_config *cfg, uint8_t *csr_clk_indx)
{
	uint32_t clock_rate;
	int ret;

	if (cfg->clock != NULL) {
		ret = clock_control_get_rate(cfg->clock, cfg->mac_clk, &clock_rate);
		if (ret < 0) {
			LOG_ERR("Failed to get MAC CSR clock rate (%d)", ret);
			return ret;
		}

		ret = dwmac_mdio_csr_clk_idx_from_rate(clock_rate, csr_clk_indx);
		if (ret == 0) {
			return 0;
		}

		LOG_WRN("Unsupported MAC CSR clock %u Hz for MDIO clock derivation", clock_rate);
	}
...
static int dwmac_mdio_init(const struct device *dev)
{
	...
	ret = dwmac_mdio_resolve_csr_clk_idx(cfg->mac_dev->config, &data->csr_clk_indx);
	if (ret < 0) {
		return ret;
	}
