...
#define TX_AVAIL_WAIT K_MSEC(100)
...
static int dwmac_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	NET_PKT_FRAG_FOR_EACH(pkt, frag) {
		...
		// LOG_HEXDUMP_DBG(frag->data, frag->len, "tx frag data");
	...
	for (d_idx = p->rx_desc_tail; d_idx != p->rx_desc_head;
		...
		if ((des0 & RDES0_LS) != 0U) {
			if ((des0 & RDES0_ES) == 0U) {
				...
				LOG_ERR("rx pkt len/frags=%zu/%u", net_pkt_get_len(p->rx_pkt),
					net_pkt_get_nbfrags(p->rx_pkt));
				LOG_ERR("DES0 = 0x%08x, DES1 = 0x%08x, DES2 = 0x%08x, DES3 = 0x%08x",
					d->des0, d->des1, d->des2, d->des3);
				eth_stats_update_errors_rx(p->iface);
				net_pkt_unref(p->rx_pkt);
			}
			p->rx_pkt = NULL;
...
static void dwmac_rx_refill(const struct device *dev)
{
	...
	LOG_DBG("desc sem/head/tail=%d/%d/%d %s", k_sem_count_get(&p->free_rx_descs), p->rx_desc_head,
		p->rx_desc_tail, k_is_in_isr() ? "ISR" : "thread");
}
...
void dwmac_isr(const struct device *dev)
{
	...
	if ((status & DWMAC_DMASR_RBUS) != 0) {
		LOG_DBG("receive buffer unavailable (0x%08x)", status);
		while (k_sem_take(&p->free_rx_descs, K_NO_WAIT) == 0) {
			dwmac_rx_refill(dev);
		}
	}
}
...
int dwmac_probe(const struct device *dev)
{
	...
	LOG_INF("hw_feature: 0x%08x", p->feature0);
	/* early IP versions don't support this register */
	if (p->feature0 != 0) {
		LOG_INF("10/100 Mbps support: %s",
			(p->feature0 & DWMAC_HWFR_10_100) ? "yes" : "no");
		LOG_INF("1000 Mbps support: %s", (p->feature0 & DWMAC_HWFR_1000) ? "yes" : "no");
		LOG_INF("Half-duplex support: %s", (p->feature0 & DWMAC_HWFR_HD) ? "yes" : "no");
		LOG_INF("Expanded DA Hash Filter: %s",
			(p->feature0 & DWMAC_HWFR_HASHF) ? "yes" : "no");
		LOG_INF("Multiple MAC Addr Reg: %s",
			(p->feature0 & DWMAC_HWFR_ADDMAC) ? "yes" : "no");
		LOG_INF("PCS register support: %s", (p->feature0 & DWMAC_HWFR_PCS) ? "yes" : "no");
		LOG_INF("SMA(MDIO) register support: %s",
			(p->feature0 & DWMAC_HWFR_SMA) ? "yes" : "no");
		LOG_INF("PMT Remote Wakeup support: %s",
			(p->feature0 & DWMAC_HWFR_PMTW) ? "yes" : "no");
		LOG_INF("PMT Magic Packet support: %s",
			(p->feature0 & DWMAC_HWFR_PMTM) ? "yes" : "no");
		LOG_INF("RMON support: %s", (p->feature0 & DWMAC_HWFR_RMON) ? "yes" : "no");
		LOG_INF("IEEE 1588-2002 support: %s",
			(p->feature0 & DWMAC_HWFR_TS2002) ? "yes" : "no");
		LOG_INF("IEEE 1588-2008 support: %s",
			(p->feature0 & DWMAC_HWFR_TS2008) ? "yes" : "no");
		LOG_INF("TX Checksum Offload support: %s",
			(p->feature0 & DWMAC_HWFR_TX_CHK) ? "yes" : "no");
		LOG_INF("RX Checksum Offload support: %s",
			(p->feature0 & DWMAC_HWFR_RXCHKV1) ? "yes" : "no");
		LOG_INF("RX Checksum Offload v2 support: %s",
			(p->feature0 & DWMAC_HWFR_RXCHKV2) ? "yes" : "no");
		LOG_INF("2K FIFO support: %s", (p->feature0 & DWMAC_HWFR_FIFO2K) ? "yes" : "no");
		LOG_INF("Alternate Descriptor support: %s",
