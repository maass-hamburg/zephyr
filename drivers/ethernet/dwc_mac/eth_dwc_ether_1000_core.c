...
static int dwmac_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	LOG_DBG("pkt len/frags=%zu/%d", net_pkt_get_len(pkt), net_pkt_get_nbfrags(pkt));

	d_idx = p->tx_desc_head;
	frag = pkt->buffer;
	des0_flags = TDES1_TCH | TDES0_IC;
	...
	do {
		...
		des0_flags = TDES0_OWN | TDES1_TCH | TDES0_IC;
	...
	d->des0 = TDES0_OWN | TDES0_FS | TDES1_TCH | TDES0_IC;
...
{
	REG_WRITE(DWMAC_MACA0HR, (addr[5] << 8) | addr[4]);
	REG_WRITE(DWMAC_MACA0LR, (addr[3] << 24) | (addr[2] << 16) | (addr[1] << 8) | addr[0]);
}
